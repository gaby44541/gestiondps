<div class="configDelais view col-lg-12 col-md-12 columns content">
	<h1>
		<?= __('ConfigDelai') ?>
		<?= $this->element('buttons',['controller'=>'configDelais','action_id'=>$configDelai->id]) ?>
	</h1>
	<table class="vertical-table table table-striped">
		<tr>
			<th><?= __('Id') ?></th>
			<td><?= $this->Number->format($configDelai->id) ?></td>
		</tr>
		<tr>
			<th><?= __('Indice') ?></th>
...
<?php if (!empty($configDelai->dispositifs)): ?>
	<table cellpadding="0" cellspacing="0" class="table table-striped" id="Dispositifs">
		...
		<tbody>
<?php foreach ($configDelai->dispositifs as $dispositifs): ?>
			<tr>
